...
var TermsArea = function (_Component) {
	...
	function TermsArea() {
		...
		return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = TermsArea.__proto__ || Object.getPrototypeOf(TermsArea)).call.apply(_ref, [this].concat(args))), _this), _this.findTermLabelStyle = function (_label, total_labels) {
			var currentLabel = _label ? _label.toLowerCase() : '';
			var capitalizedLabel = currentLabel.charAt(0).toUpperCase() + currentLabel.slice(1);
			var label = total_labels[currentLabel] || total_labels[capitalizedLabel];
	...
	_createClass(TermsArea, [{
		...
		value: function render() {
			...
			var terms = enteredTerms.map(function (term) {
				return _react2.default.createElement(_Term2.default, {
					...
					label: term.label,
					labelStyle: _this2.findTermLabelStyle(term.label, labels)
				});
			});
...
TermsArea.propTypes = _defineProperty({
